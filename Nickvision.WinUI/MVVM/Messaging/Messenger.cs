...
namespace Nickvision.WinUI.MVVM.Messaging;

/// <summary>
/// A class for sending messages (triggering actions) between ViewModels.
/// </summary>
public sealed class Messenger
{
    ...
    public async void Send(string messageName, object? parameter)
    {
        if (!_asyncLookup.ContainsKey(messageName))
        {
            return;
    ...
    public async Task<bool> SendAsync(string messageName, object? parameter)
    {
        if (!_asyncLookup.ContainsKey(messageName))
        {
            return false;
